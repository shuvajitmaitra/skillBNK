...
import {
  TemplateApiResponse,
  TemplateItem,
} from '../../types/documents/templateTypes';
import TemplateCard from '../../components/Documents/Template/TemplateCard';
...
export default function TemplatesScreen() {
  ...
  const [contents, setContents] = useState<TemplateItem[]>([]);
  ...
  const filterDocuments = useCallback(
    (items: TemplateItem[], query: string) => {
  ...
  const handleNavigation = useCallback((item: TemplateItem) => {
    navigate('ProgramStack', {
      screen: 'TemplateDetailsScreen',
      params: {item},
    });
  }, []);
  ...
  const fetchPage = useCallback(
    async (page = 1, isLoadMore = false, passedDate?: string) => {
      try {
        ...
        const res = await axiosInstance.get<TemplateApiResponse>(
        ...
        const newItems = res?.data?.templates ?? [];
  ...
  const keyExtractor = useCallback((item: TemplateItem) => item._id, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subHeading}>Easy Access to your templates</Text>
