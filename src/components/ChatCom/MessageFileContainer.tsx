...
const MessageFileContainer = ({
  ...
  const imageRender = ({item}: {item: {url: string}}) => {
    const {aspectRatio = 1} = imageDimensions[item?.url] || {};
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          setViewImage &&
          setViewImage(files.map((i, idx) => ({uri: i.url, index: idx})))
        }>
        <Image
          source={{uri: item?.url}}
          style={[
            styles.image,
            {aspectRatio: aspectRatio || 1}, // Use calculated or fallback aspect ratio
          ]}
          onLoad={({nativeEvent: {source}}) =>
            handleImageLayout(item.url, source.width, source.height)
          }
          onError={() => {
            console.log(`Failed to load image: ${item.url}`);
            setImageDimensions(prev => ({
              ...prev,
              [item.url]: {aspectRatio: 1}, // Fallback on error
            }));
          }}
        />
      </TouchableOpacity>
